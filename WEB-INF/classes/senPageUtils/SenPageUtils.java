...
import senpageUtils.AccessDBUtils;


public class SenpageUtils
{
 public ResourceBundle bundle = ResourceBundle.getBundle("senpage");
 ...
 ///////////////////////// senpageSetRpt ///////////////////////////////
 public synchronized String senpageSetRpt(ServletContext context, HttpServletRequest request, HttpServletResponse response, HttpSession senpageSession)
 {
  this.context = context;

  // this.sessionNav =(String)rtcmSession.getAttribute("sessionNav");
  String sessionUser = (String)senpageSession.getAttribute("senpageUser");
  String sessionGrp = (String)senpageSession.getAttribute("senpageGroup");

  String requestType = request.getParameter("requestType");
  StringBuffer buff = new StringBuffer("");
  System.out.println("33 - curr request_type: " + requestType);
  if(requestType.equals("main"))
  {
   buff.append(requestType);
   //buff.append(this.setPageData("senpageSplash"));
  }
  else if(requestType.equals("senpageAutoComplete"))
  {
		 buff.append("requestType: "+requestType);
			System.out.println("42");
   //buff.append(this.senpageUtilSsetJasonOut(request.getParameter("category")));
   //buff.append(this.setPageData("latticeBrand"));
  }
		else if(requestType.equals("test"))
  {
			System.out.println("48 - exec test: "+request.getParameter("term"));
   //buff.append(request.getParameter("term"));
   //buff.append("<br/>res: got term");
			ArrayList<String> list = testDB(request.getParameter("term"));
   //String list = testDB(request.getParameter("term"));
   try
   {
  ...
  else if(requestType.equals("senpageCurrUser"))
  {
   buff.append(sessionUser + "~~" + sessionGrp);
  }
  else if(requestType.equals("senpageSetPageInfo"))
  {
   buff.append(this.pageInfo);
  }
  else if(requestType.equals("senpageSignOff"))
  {
   senpageSession.removeAttribute("senpageUser");
   senpageSession.invalidate();
  }
  else
  {
  ...
  return buff.toString();
	...
	/* //////////////////////////// testDB//////////////*/
 public synchronized  ArrayList<String> testDB(String term)
 //public synchronized  String testDB(String term)
 {
  StringBuffer buff=new StringBuffer("");
  ...
  this.adbu.vecParams = new Vector();
  termBuff.append("%");
  termBuff.append(term);
  termBuff.append("%");
 
  this.adbu.vecParams.add(termBuff.toString());
  String sql="select language from SENPAGETEST where lower(language) like lower(?)";
  this.adbu.accessDB(this.context, sql, this.adbu.vecParams, "EQ", "h2");
  ...
  if(adbu.dbException == null)
  {
   try
   {
    ...
    {
     list.add(this.adbu.res.getString(1));
     //buff.append(this.adbu.res.getString(1));
    }
   }
   catch (Exception ex)
   {
    list.add(this.setErrorPage("testDB - 100: "+this.adbu.dbException));
   }
   finally
   {
  ...
  {
   this.adbu.closeAllConns();
   list.add(this.setErrorPage("testDB - 110: "+this.adbu.dbException));
  }
  return list;
  //return buff.toString();
 }
 /*/////////////////////////////////////////////////////// //setJsonout*/ ///////////////////////////////
 public synchronized String senpageUtilSsetJasonOut(String category)
 {
